...
window.addEventListener('DOMContentLoaded', function () {
  ...
  Object(_modules_pictureSize__WEBPACK_IMPORTED_MODULE_8__["default"])('.sizes-block'); // accordion('.accordion-heading', '.accordion-block'); //Для первого и второго способа 

  Object(_modules_accordion__WEBPACK_IMPORTED_MODULE_9__["default"])('.accordion-heading'); //Для третьего способа
...
var accordion = function accordion(triggersSelector) {
  ...
  btns.forEach(function (btn) {
    btn.addEventListener('click', function (e) {
      var cTarget = e.currentTarget; // Пришлось костыли ставить так как toggle не работает если с самого начала небудет стоять класс с которым он будет работать.
...
var calc = function calc(size, material, options, promocode, result, modalState) {
  ...
  var calcForm = function calcForm(event, element, prop) {
...
var checkTextInputs = function checkTextInputs(selector) {
  ...
  txtInputs.forEach(function (input) {
    input.addEventListener('input', function (e) {
      ...
      var filteredText = inputText.replace(/[^Аа-яё 0-9]/g, ''); // Обновляем значение поля ввода
...
var drop = function drop() {
  ...
  var fileInputs = document.querySelectorAll('[name="upload"]'); // Добавляем обработчики событий 'dragenter', 'dragleave', 'dragover', 'drop' на каждый из них. 
...
var filter = function filter() {
  ...
  var funcFilter = function funcFilter(markSelector, markPicture) {
    menu.addEventListener('click', function (e) {
      ...
      if (target.nodeName === 'LI' && target == markSelector) {
        ...
        if (markPicture === no) {
          ...
          markPicture.forEach(function (item) {
...
var forms = function forms(modalState) {
  ...
  var bindPostData = function bindPostData() {
    forms.forEach(function (item) {
      item.addEventListener('submit', function (e) {
        ...
        Object(_services_requests__WEBPACK_IMPORTED_MODULE_6__["postData"])(api, formData).then(function (data) {
          ...
          // Очистить все инпуты
          clearInput(); // Удалить блок статуса отправки данных на сервер
          ...
          }, 5000); // Скрыть все модальные окна

          setTimeout(function () {
            document.querySelectorAll('div[data-modal]').forEach(function (item) {
              return item.style.display = 'none';
            });
...
var modals = function modals() {
  ...
  bindModal('.button-design', '.popup-design', '.popup-design button.popup-close');
  bindModal('.button-consultation', '.popup-consultation', '.popup-consultation button.popup-close');
  bindModal('.fixed-gift', '.popup-gift', '.popup-gift button.popup-close', true);
...
var pictureSize = function pictureSize(imgSelector) {
  ...
  var showImg = function showImg(block) {
    ...
    var img = block.querySelector('img'); // Перем путь каритнки и отрезаем последние 4 символа и добавляем свои.

    img.src = img.src.slice(0, -4) + '-1.png'; // Перебираем все p в блоке и скрываем их.
...
var scrolling = function scrolling(upSelector) {
  var upElem = document.querySelector(upSelector); // Плавное появление кнопки вверх
  ...
  links.forEach(function (link) {
    ...
    link.addEventListener('click', function (event) {
      event.preventDefault(); // Далее вычисляется текущая позиция скролла (document.documentElement.scrollTop) и расстояние от блока, на который ссылается якорь, до верхней части экрана (toBlock.getBoundingClientRect().top).

      var widthTop = document.documentElement.scrollTop,
          hash = this.hash,
          toBlock = document.querySelector(hash).getBoundingClientRect().top,
          start = null; // Далее, для плавной прокрутки страницы, используется функция requestAnimationFrame(). 
...
var showMoreStyles = function showMoreStyles(triggerSelector, parentSelector) {
  var triggerButton = document.querySelector(triggerSelector);
